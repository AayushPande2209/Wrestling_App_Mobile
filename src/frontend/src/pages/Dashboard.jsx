...
const TABS = ['OVERVIEW', 'TIMELINE', 'RECORDS']

function StatBox({ label, value, highlight }) {
  return (
    <div className="bg-[#0a0a0a] border border-[#1a1a1a] p-4 md:p-5">
      <div className="text-[10px] font-display tracking-[0.15em] text-[#aaa] mb-2">{label}</div>
      <div className={`font-mono text-2xl md:text-3xl font-bold tracking-tight ${highlight ? 'text-[#e8712a]' : 'text-[#f0f0f0]'}`}>
        {value}
      </div>
    </div>
  )
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stat boxes — always visible above tabs */}
              ...
              <div className="space-y-2">
                {feed.map((item, i) => (
                  <div key={i} className="flex items-baseline gap-3 font-mono text-[11px]">
                    ...
                    <span className="text-[#e8712a] shrink-0">{item.name}</span>
            ...
            <div className="flex flex-col md:flex-row gap-3">
              {[
                ...
                <button
                  key={to}
                  onClick={() => navigate(to)}
                  className="w-full md:w-auto px-5 py-2.5 border border-[#1e1e1e] font-display text-[10px] tracking-[0.18em] text-[#888] hover:border-[#e8712a] hover:text-[#e8712a] transition-colors min-h-[44px]"
                >
                  {label}
                </button>
              ))}
      ...
      {/* ── TIMELINE tab ── */}
      {activeTab === 'TIMELINE' && <TimelineView uid={uid} />}

      {/* ── RECORDS tab ── */}
      {activeTab === 'RECORDS' && <RecordsView uid={uid} />}
    </div>
  )
}
